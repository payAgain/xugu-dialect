...
**I-009 / I-010：** 产品延后行已关闭；Definition A open **延后** 仅 `A-XCUT-012`（Ship）。诚实 live/known-limit：[`contracts/production-regression-baseline.md`](../contracts/production-regression-baseline.md)（A-FUN-021 = **known-limit-documented**；A-TYP-014/016/017 与 Batch A 在 live 未 PASS 前保持 known-limit）。用户说明：[04-feature-matrix.md § I-009](user-guide/04-feature-matrix.md#i-009-deferred-closure终态)、[§ I-010](user-guide/04-feature-matrix.md#i-010-quality-completion退出清单)、[05-troubleshooting.md §13–15](user-guide/05-troubleshooting.md#13-encrypt-by--partition--catalog-工具边界)。
