# Error Journal (migrated)

> Moved from `docs/error-journal.md` on 2026-07-22.
> Prefer capturing new lessons in `.trellis/spec/` and session journals.
...
### Symptom
I-001 Accept 后独立工程（已归档删除）实库复测 20/22：HQL 分页生成 ANSI `OFFSET/FETCH` 失败；`hbm2ddl.validate` 报 missing sequence。本仓 Phase 证据与 VERIFY PASS 均曾为绿。
...
- 框架：见完整分析 `(removed at harness→Trellis migration; lesson retained in workspace error-journal)`（矩阵 entrypoint 字段、test/reviewer 测错层=MAJOR、Accept/Ship 分档、VERIFY 升级）
...
### Related Files / Tasks
- `(removed at harness→Trellis migration; lesson retained in workspace error-journal)`
- `.trellis/tasks/archive/2026-07/i-002-hql-pagination-sequence-metadata/research/independent-it-2026-07/TEST-REPORT.md`
