...
| A-TYP-014 | Types | INTERVAL | Map Hibernate duration/interval if Dialect exposes | `reference/sql/datatype/datetime.md` | 可实现 | **I-010/P-002** | ✅ **known-limit-documented** — entity ORM path (`XuguIntervalJdbcType` + `intervalEntityOrmRoundTrip_A_TYP_014`) for DURATION/INTERVAL_SECOND; 11 other subtypes tooling-only; covered-live pending live IT; baseline [`production-regression-baseline.md`](production-regression-baseline.md) § A-TYP-014 |
...
| A-TYP-016 | Types | XML | Map SQLXML / XML type | `reference/sql/datatype/xml.md` | 可实现 | **I-010/P-003** | ✅ **known-limit-documented** — entity ORM path (`XuguXmlJdbcType` + `xmlEntityOrmRoundTrip_A_TYP_016`) for `String`+`@JdbcTypeCode(SQLXML)`; `java.sql.SQLXML` not recommended; native IT retained; covered-live pending live IT; baseline § A-TYP-016 |
| A-TYP-017 | Types | Geometric / spatial | Map geometry types | `reference/sql/datatype/geometric.md` | 可实现 | **I-010/P-004** | ✅ **known-limit-documented** — POINT/GEOMETRY→POINT entity ORM (`XuguPointJdbcType` + `pointEntityOrmRoundTrip_A_TYP_017`); LINE/LSEG/BOX/PATH/POLYGON/CIRCLE native/tooling only; simple 2D (not PostGIS); covered-live pending live IT; pair A-FUN-020; baseline [`production-regression-baseline.md`](production-regression-baseline.md) § A-TYP-017 |
...
| ID | Domain | Capability | Hibernate/Dialect surface (what apps expect) | Xugu doc ref | Status | Target Phase | Acceptance hint |
|---|---|---|---|---|---|---|---|
| A-PAG-001 | Pagination | LIMIT only | `LimitHandler` + HQL/Criteria `SqlAstTranslator` limit without offset | `reference/sql/select/resultset-restricted.md` | 可实现 | P-004 | ✅ `LIMIT ?` via XuguLimitHandler **and** `XuguSqlAstTranslator` (I-002/P-001); with locks: FOR UPDATE → LIMIT (XuGu grammar) |
| A-PAG-002 | Pagination | LIMIT + OFFSET | `supportsLimitOffset` / HQL `setFirstResult`+`setMaxResults` | `reference/sql/select/resultset-restricted.md` | 可实现 | P-004 | ✅ stable form `LIMIT count [OFFSET offset]` (**not** ANSI `OFFSET…FETCH`); I-002/P-001 AST path; with locks: **FOR UPDATE → LIMIT → WAIT** |
...
| A-PAG-004 | Pagination | TOP syntax | Alternate top-N | `reference/sql/select/resultset-restricted.md` (#top) | 可实现 | **I-009/P-009** | **known-limit** — TOP ⊥ LIMIT; `XuguPaginationAlternativesSupport`; LimitHandler unchanged |
| A-PAG-005 | Pagination | ANSI FETCH FIRST | `FETCH FIRST n ROWS ONLY` | — (not documented under resultset-restricted) | 文档不允许 | P-004 | ✅ not emitted (unit + IT assert); I-002/P-001 closed E19132 `unexpected OFFSET` on HQL page path |
| A-PAG-006 | Pagination | ROWNUM pagination | Oracle-style ROWNUM wrappers | `reference/sql/select/select.md` (#ROWNUM) | 可实现 | **I-009/P-009** | **known-limit** — alternate native path; LIMIT remains default |
| A-LCK-001 | Locks | FOR UPDATE | Pessimistic write lock clause | `reference/sql/select/select.md` (#opt_for_update_clause) | 可实现 | P-004 | ✅ `for update` + IT; combo with page: FOR UPDATE before LIMIT (LimitHandler + SqlAstTranslator) |
| A-LCK-002 | Locks | FOR UPDATE OF columns | Optional column list | `reference/sql/select/select.md` | 可实现 | P-004 | ✅ `for update of …` + IT |
| A-LCK-003 | Locks | Lock wait / NOWAIT | `LockOptions.NOWAIT` / timeout | `reference/sql/select/select.md` (`opt_wait`: NOWAIT / WAIT / WAIT ms); also `reference/object/table/lock.md` | 可实现 | P-004 | ✅ ms pass-through; with page: trailing WAIT after LIMIT (I-002/P-001 IT) |
...
| A-LCK-006 | Locks | LOCK TABLE (explicit) | Rare Dialect helper for table locks | `reference/object/table/lock.md` | 可实现 | **I-009/P-009** | ✅ `XuguLockTableSupport` native SQL — not JPA LockMode path |
...
| A-IDN-005 | Identity | Identity mode session params | Dialect-specific identity_mode knobs | `reference/system-configuration-parameter/session-parameter/identity_mode.md`, `reference/system-configuration-parameter/xugu.ini/compatible/def_identity_mode.md` | 可实现 | **I-009/P-009** | ✅ `XuguIdentityModeSupport` — NULL/ZERO-as-auto-increment (v12.0.6+) |
| A-SEQ-001 | Sequence | CREATE SEQUENCE | `SequenceSupport.getCreateSequenceString` | `reference/object/sequence.md` | 可实现 | P-005 | ✅ `create sequence … [start with N increment by M]`; I-002/P-002: `getQuerySequencesString`→`select * from all_sequences` + Xugu extractor (`seq_name`/`min_val`/`max_val`/`step_val`) so `hbm2ddl.auto=validate` sees existing sequences |
...
| A-FUN-003 | Functions | length / char_length | string length | `reference/function/string-functions/length.md`, `reference/function/string-functions/char_length.md` | 可实现 | P-006 / **I-010/P-008** | ✅ **known-limit-documented** — independent HQL Session `lengthFamilyHqlSession_A_FUN_003` present; covered-live pending live IT PASS; baseline § A-FUN-003 |
...
| A-FUN-005 | Functions | trim / ltrim / rtrim | trim family | `reference/function/string-functions/trim.md`, `ltrim.md`, `rtrim.md` | 可实现 | P-006 / **I-010/P-008** | ✅ **known-limit-documented** — independent HQL Session `trimFamilyHqlSession_A_FUN_005` present; covered-live pending live IT PASS; baseline § A-FUN-005 |
| A-FUN-006 | Functions | replace / locate / position | search/replace | `reference/function/string-functions/replace.md`, `locate.md`, `position.md` | 可实现 | P-006 / **I-010/P-008** | ✅ **known-limit-documented** — independent HQL Session `replaceLocateHqlSession_A_FUN_006` present; covered-live pending live IT PASS; baseline § A-FUN-006 |
| A-FUN-007 | Functions | coalesce / nullif / nvl | null-handling | `reference/sql/expression/function.md` (COALESCE), `reference/function/flow-control-functions/nullif.md`, `reference/function/string-functions/nvl.md`, `reference/function/flow-control-functions/ifnull.md` | 可实现 | P-006 / **I-010/P-008** | ✅ **known-limit-documented** — independent HQL Session `coalesceNvlHqlSession_A_FUN_007` present; covered-live pending live IT PASS; baseline § A-FUN-007 |
...
| A-FUN-009 | Functions | round / floor / ceil / trunc | rounding | `reference/function/mathematical-functions/round.md`, `floor.md`, `ceil.md`, `trunc.md` | 可实现 | P-006 / **I-010/P-008** | ✅ **known-limit-documented** — independent HQL Session `roundTruncHqlSession_A_FUN_009` present; covered-live pending live IT PASS; baseline § A-FUN-009 |
...
| A-FUN-019 | Functions | regexp_* | regex HQL | `reference/function/string-functions/regexp_like.md`, `regexp_replace.md`, `regexp_substr.md` | 可实现 | **I-009/P-006** / **I-010/P-007** | ✅ **covered-live** — registered + native IT + HQL Session `regexp_like`/`regexp_replace`/`regexp_substr` (`regexpFunctionsHqlSession_A_FUN_019`); no `regexp_instr` claim |
| A-FUN-020 | Functions | geometric functions | spatial HQL | `reference/function/geometric-functions/` | 可实现 | **I-009/P-004** / **I-010/P-006** | ✅ **covered-live** — native subset + HQL Session `area`/`center`/`point` (`geometricFunctionsHqlSession_A_FUN_020`); pair A-TYP-017 |
...
| A-SCH-003 | Catalog | Catalog (database) qualifier | `catalog.schema.table` | `reference/object/database.md` | 可实现 | **I-009/P-008** | ✅ P-008 known-limit: JDBC `current_db()` alignment; object names stay `schema.table` (DATABASE not session-SET) |
...
| A-SCH-017 | Indexes | Advanced index types | Partial/functional/spatial indexes | `reference/object/indexes.md` | 可实现 | **I-009/P-008** | ✅ P-008 known-limit: functional + BITMAP native DDL; schema export stays B-tree (A-SCH-016) |
